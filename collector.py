...
class MiioCollector:
    def __init__(self, config_path='config.json', db_path=None):
        ...
        self.db_path = db_path or os.getenv('DB_PATH', 'data.db')
    ...
    def collect_device_data(self, device_config):
        ...
        try:
            ...
            if rows_to_insert:
                self.insert_sensor_rows(rows_to_insert)

        except Exception as e:
            logger.error(f"连接设备 {device_config['name']} 失败: {e}")

    def insert_sensor_rows(self, rows):
        for attempt in range(LOCK_RETRY_TIMES):
            try:
                ...
                logger.warning(f"数据库忙，{wait_seconds:.1f} 秒后重试写入")
        ...
        for attempt in range(LOCK_RETRY_TIMES):
            try:
                ...
                if 'locked' not in str(e).lower() or attempt == LOCK_RETRY_TIMES - 1:
                    raise
                wait_seconds = LOCK_RETRY_BASE_DELAY * (2 ** attempt)
                logger.warning(f"数据库忙，{wait_seconds:.1f} 秒后重试清理")
                time.sleep(wait_seconds)
